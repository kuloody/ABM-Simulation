...
class SchellingAgent(Agent):
    ...
    def changeState(self):
        ...
        if self.yellowState > 3 and self.disruptiveTend <= compute_ave_disruptive(self.model):
            self.type = 1
        ...
        if self.redState > 2 and self.disruptiveTend <= compute_ave_disruptive(self.model) :
            ...
            self.set_start_math()
            return
        if self.greenState > 3 and self.disruptiveTend >= compute_ave_disruptive(self.model):
            self.type = 2
            self.redState = 0
            self.yellowState += 1
            self.greenState = 0
...
class Schelling(Model):
    ...
    def __init__(self, height=6, width=6, quality=1, Inattentiveness=0, control=3, hyper_Impulsive=0, AttentionSpan=0):
        ...
        for cell in self.grid.coord_iter():
            ...
            if hyper_Impulsive == 1:
                hyper_Impulsive_list = [1, 2, 3, 4] * 30 + [5, 6] * 70
            else:
    ...
    def step(self):
        ...
        if self.schedule.steps == 1200.0 or self.running == False:
            ...
            dataAgent.to_csv(
                '/home/zsrj52/Downloads/SimClass/Simulations19-08-2020/Simulation08-lowControl.csv')
