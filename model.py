...
class SimClassAgent(Agent):
    ...
    def yellowStateCange(self):
        ...
        if self.model.Inattentiveness == 0 and self.behave > self.agent_state:
            ...
            self.greenState = 0
            return 1
        if compute_SD(self.model, self.disruptiveTend) and self.model.control >= self.agent_state and self.behave_2 <= self.agent_state:
            if self.model.learning > 0:
                self.model.learning -= 1
            self.type = 2
            if self.model.distruptive > 0:
...
class SimClass(Model):
    ...
    def __init__(self, height=6, width=6, quality=1, Inattentiveness=0, control=3, hyper_Impulsive=0, AttentionSpan=0):
        ...
        #agent_start_math = gen_random()
        print('Meian',np.median(agent_start_math))
        print('Mean ***',agent_start_math.mean())
    ...
    def step(self):
        ...
        if self.schedule.steps == 8550.0 or self.running == False:
            ...
            dataAgent.to_csv(
                '/home/zsrj52/Downloads/SimClass/Simulations-106/Simulation1-Heyperactivity.8.csv')
